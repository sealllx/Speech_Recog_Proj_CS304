...
class Path:
    def __init__(self, sequence,node,left_seq, dtw=0):
        ...
        self.node=node
class Lextree:
    def __init__(self, trained_hmms,HMM_models,silence_model,silence=True):
        self.states = []
        self.trained_hmms = HMM_models
        self.silence_hmm= silence_model
    ...
    def create_states_for_digits(self):
       ...
       # non_emitting_states.append(new_non_emitting)  # 将这轮的非发射状态添加到列表中，以供下一轮使用
        if self.add_silence:
            ...
            transition_matrix=self.silence_hmm.transition_probs
            prev_state=None
            for state_index in range(self.silence_hmm.n_states):
                state_name = f"silence_{state_index}"
                means = self.silence_hmm.means[state_index]
                variances = self.silence_hmm.vars[state_index]
                state = State(name=state_name, means=means, variances=variances)
               # self.states.append(state)
                # 添加自循环
...
def best_path_improved(whole_sequence,Lextree, pruning_val):
    ...
    #    max_err=max([k.dtw for k in new_paths])
        ...
        thresh=sorted([k.dtw for k in new_paths])[:20][-1]
...
# 使用
    ...
    lextree = Lextree(HMM_list,HMM_models=HMM_list,silence_model=silence_HMM,silence=True)
    return lextree
